...
pub const ForkChoice = struct {
    ...
    fn aggregateUnlocked(self: *Self, state_opt: ?*const types.BeamState, slot_filter: ?[]const types.Slot) ![]types.SignedAggregatedAttestation {
        ...
        // ─── Phase 1: snapshot ──────────────────────────────────────
        // Collect in-scope att_data keys under signatures_mutex and
        // deep-clone only those entries so we do not sszClone every stale
        // proof retained in the live maps.
...
/// Returns true when `att_data.slot` is in the optional aggregate slot window.
...
/// Collect `AttestationData` keys to snapshot. Gossip groups are always
/// included; payload-only groups are included only when they have at least
/// two pending proofs, because a single child can already pass through
/// without a recursive aggregate prove.
fn collectAggregationSnapshotKeys(
    ...
    var gossip_keys = std.AutoHashMap(types.AttestationData, void).init(allocator);
    defer gossip_keys.deinit();

    var sig_it = fork_choice.attestation_signatures.iterator();
    while (sig_it.next()) |entry| {
        if (!aggregationSlotAllowed(entry.key_ptr.slot, slot_filter)) continue;
        try gossip_keys.put(entry.key_ptr.*, {});
        const gop = try seen.getOrPut(entry.key_ptr.*);
        if (!gop.found_existing) try out_keys.append(allocator, entry.key_ptr.*);
    }

    var payload_it = fork_choice.latest_new_aggregated_payloads.iterator();
    while (payload_it.next()) |entry| {
        if (!aggregationSlotAllowed(entry.key_ptr.slot, slot_filter)) continue;
        if (gossip_keys.contains(entry.key_ptr.*)) continue;
...
/// Deep-clone one attestation-data entry from `src_list` into `dst`.
...
/// Taken under `signatures_mutex` using slot-window scoping: only in-window
/// `att_data` keys, plus payload-only groups with at least two proofs, are
/// cloned.
