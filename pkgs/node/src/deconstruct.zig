...
//! attestation's Type-1 out of the Type-2, merge it with disjoint local partial proofs for the same
//! data into one higher-coverage Type-1, write it into the pending pool
//! (`latest_new_aggregated_payloads`), and return the combined aggregate for the aggregator role to
//! gossip.
//!
//! This refills the pending pool that block PROPOSAL selects aggregated attestations from. It is a
//! liveness path in a multi-node network: fork-choice weight is eager (`AttestationTracker`, fed by
//! `onAttestation(is_from_block=true)`), but PROPOSAL coverage comes from the pool. A node that
//! imports a block without separately receiving its gossip aggregate would otherwise have nothing to
//! re-propose for that data, producing blocks with no aggregated attestations (coverage=none) — so
//! the chain stops gaining justification weight and never justifies/finalizes.
//!
...
/// Max children per recursive `TypeOneMultiSignature.aggregate` call; exceeding the prover's
...
pub fn deconstructCompute(
    ...
    // SSZ edge only — non-fatal: recover nothing.
