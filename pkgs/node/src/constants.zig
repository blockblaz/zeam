...
// and trigger a one-shot full-fanout status refresh.
//
// In one observed incident a node was ~254 wall slots behind, but every cached
// `latest_status.head_slot` it knew about topped out at 45 (a peer that was
// itself stuck behind broken gossip ingress at the same network position).
// `findBestCatchUpPeerStatus` correctly picked the best-of-stale option each
// slot, fired `blocks_by_root` to it, got a stale response, and made no
// progress — for 25+ minutes — because the rotating status-refresh batch
// (`SYNC_STATUS_REFRESH_PEERS_PER_TICK = 8`, full rotation = ~8 slots over a
// 50-peer mesh) was the only path that could ever learn that some
// non-batch-current peer might have a fresher head. Sixteen slots = 64 s
// tolerance: anything below this is normal proactive-catch-up territory;
// anything above signals "the visible mesh is not on the chain tip."
...
// "stuck mesh cluster" detector fires. Rate-limits the otherwise-unbatched RPC
// burst so that, in the pathological case where a genuine network-wide pause
// keeps us stuck, we don't blast every connected peer with status requests
// every interval and re-create the en-masse RPC timeout cascade that motivated
// the original batching.
//
