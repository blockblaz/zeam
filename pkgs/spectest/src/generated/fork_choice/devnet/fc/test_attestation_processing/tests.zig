...
pub const fixture_count: usize = 6;

test "devnet fc test_attestation_processing test_attestation_accumulation_full_validator_set" {
    ...
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_processing/test_attestation_accumulation_full_validator_set.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_processing test_attestation_superseding_same_validator" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_processing/test_attestation_superseding_same_validator.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_processing test_attestations_move_to_known_between_blocks" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_processing/test_attestations_move_to_known_between_blocks.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_processing test_extended_chain_attestation_superseding_pattern" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_processing/test_extended_chain_attestation_superseding_pattern.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_processing test_proposer_attestation_appears_in_latest_new" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_processing/test_proposer_attestation_appears_in_latest_new.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_processing test_slot_gaps_with_attestation_superseding" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
