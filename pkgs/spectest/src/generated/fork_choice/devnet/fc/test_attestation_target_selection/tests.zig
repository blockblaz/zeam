...
pub const fixture_count: usize = 5;

test "devnet fc test_attestation_target_selection test_attestation_target_advances_with_attestations" {
    ...
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_target_selection/test_attestation_target_advances_with_attestations.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_target_selection test_attestation_target_at_genesis_initially" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_target_selection/test_attestation_target_at_genesis_initially.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_target_selection test_attestation_target_justifiable_constraint" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_target_selection/test_attestation_target_justifiable_constraint.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_target_selection test_attestation_target_with_extended_chain" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_attestation_target_selection/test_attestation_target_with_extended_chain.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_attestation_target_selection test_attestation_target_with_slot_gaps" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
