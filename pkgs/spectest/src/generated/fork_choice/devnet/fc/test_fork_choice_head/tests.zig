...
test "devnet fc test_fork_choice_head test_head_advances_through_deep_chain" {
    ...
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_fork_choice_head/test_head_advances_through_deep_chain.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_fork_choice_head test_head_switches_to_heavier_fork" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_fork_choice_head/test_head_switches_to_heavier_fork.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_fork_choice_head test_head_with_deep_fork_split" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_fork_choice_head/test_head_with_deep_fork_split.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_fork_choice_head test_head_with_gaps_in_slots" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_fork_choice_head/test_head_with_gaps_in_slots.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_fork_choice_head test_head_with_large_gaps" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
    ...
    defer fixtures_dir.close();
    const Case = fork_choice.TestCase(forks.devnet, "consensus/fork_choice/devnet/fc/test_fork_choice_head/test_head_with_large_gaps.json");
    try Case.execute(test_allocator, fixtures_dir);
}

test "devnet fc test_fork_choice_head test_head_with_two_competing_forks" {
    configureSkipBehaviour();
    const fixtures_path = resolveFixturesRoot(test_allocator) catch |err| switch (err) {
        ResolveError.FixturesNotFound => return error.SkipZigTest,
        else => return err,
