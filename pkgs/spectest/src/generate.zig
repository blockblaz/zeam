...
const usage =
    ...
    "  zig run pkgs/spectest/src/generate.zig -- [options]\n" ++
    "\n" ++
    "Options:\n" ++
...
fn makeHeaderWithPrefix(allocator: Allocator, prefix: []const u8) ![]u8 {
    var list = std.ArrayListUnmanaged(u8).empty;
    errdefer list.deinit(allocator);
    var writer = list.writer(allocator);

    try writer.writeAll("// This file is generated by pkgs/spectest/src/generate.zig.\n");
    try writer.writeAll("// Do not edit manually.\n\n");
    try writer.writeAll("const std = @import(\"std\");\n");
    try writer.print("const forks = @import(\"{s}fork.zig\");\n", .{prefix});
    try writer.print("const state_transition = @import(\"{s}state_transition_runner.zig\");\n", .{prefix});
    try writer.print("const fork_choice = @import(\"{s}fork_choice_runner.zig\");\n\n", .{prefix});
    ...
    try writer.writeAll("    state_transition.configureSkipExpectedErrorFixturesFromEnv();\n");
    try writer.writeAll("    if (skip_cli_initialized) return;\n");
    try writer.writeAll("    if (parseSkipOverrideFromArgs()) |override| {\n");
    try writer.writeAll("        state_transition.setSkipExpectedErrorFixtures(override);\n");
    try writer.writeAll("    }\n");
    ...
    try writer.writeAll("        } else if (std.mem.eql(u8, arg, \"--no-skip-expected-error-fixtures\")) {\n");
    try writer.writeAll("            override = false;\n");
    ...
    try writer.writeAll("    if (equalsIgnoreCase(trimmed, \"0\") or equalsIgnoreCase(trimmed, \"false\") or equalsIgnoreCase(trimmed, \"no\") or equalsIgnoreCase(trimmed, \"off\")) return false;\n");
    try writer.writeAll("    if (equalsIgnoreCase(trimmed, \"1\") or equalsIgnoreCase(trimmed, \"true\") or equalsIgnoreCase(trimmed, \"yes\") or equalsIgnoreCase(trimmed, \"on\")) return true;\n");
...
fn writeHandlerFile(
    ...
    const header_text = try makeHeaderWithPrefix(allocator, runner_prefix);
