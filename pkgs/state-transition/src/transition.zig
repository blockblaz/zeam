...
pub fn verifySignatures(
    ...
    const attestations = signed_block.message.body.attestations.constSlice();
    ...
    const proposer_index: usize = @intCast(signed_block.message.proposer_index);
    ...
    try zeam_utils.hashTreeRoot(types.BeamBlock, signed_block.message, &block_root, allocator);

    const block_epoch: u32 = @intCast(signed_block.message.slot);
