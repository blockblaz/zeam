...
/// gathers a slice of these and hands them to
/// `verifySignaturesParallelMultiBlock`, which verifies each block's merged
/// Type-2 proof on its own thread-pool worker.
///
/// `state` is the parent state for this block (its validator set is used
/// for the pubkey lookup). `precomputed_block_root` is the block's
/// hashTreeRoot, already computed by the caller (chain.onBlocksBatch) on the
/// catch-up hot path; it is forwarded to `verifySignatures` so the per-block
/// verify skips re-hashing the block.
...
/// Multi-block batched signature verification for devnet5's single Type-2
/// proof model. Each SignedBlock carries one merged Type-2 recursive-STARK
/// proof (`SignedBlock.proof`) covering every attestation Type-1 plus the
/// proposer, so there is no per-attestation signature list to coalesce into a
/// single FFI call. "Batch" here therefore means dispatching the N independent
/// per-block Type-2 verifies across `thread_pool`.
///
/// This is sound: leanMultisig's verify path is reentrant — it reads only
/// immutable `OnceLock` globals (aggregation bytecode, poseidon) and per-call
/// state, and the zk-alloc bump-arena that makes the PROVE path non-reentrant
/// is never installed in this binary (jemalloc is the global allocator;
/// `begin_phase` is prove/benchmark-only). `PublicKeyCache.getOrPut` is
/// lock-free, and each `verifySignatures` builds its own arena from
/// `allocator` — which therefore MUST be thread-safe (a serialized GPA or
/// c_allocator; NOT an `ArenaAllocator`, whose concurrent use corrupts its
/// bookkeeping). Requires `xmss_setup_verifier()` to have run once before
/// fan-out, which it has by the time any block is verified.
///
/// All-or-nothing: the first block whose verify fails makes the whole call
/// return that error; `chain.onBlocksBatch` then falls back to per-block
/// verification to isolate which block carried the bad proof.
///
/// K=1 delegates to the single-block `verifySignatures` (no batching upside).
pub fn verifySignaturesParallelMultiBlock(
    ...
    if (tasks.len == 1) {
        return verifySignatures(allocator, tasks[0].state, tasks[0].signed_block, pubkey_cache, tasks[0].precomputed_block_root);
    }

    // Observe batch size so dashboards can correlate per-call cost with how many
    // blocks were coalesced into this dispatch.
    zeam_metrics.zeam_stf_verify_signatures_batch_size.record(@floatFromInt(tasks.len));

    // One error slot per block. ThreadPool worker functions must return `void`
    // and must not panic (a panic cannot unwind defers and would tear down a
    // worker thread), so each task records any error out-of-band; the atomic
    // flag lets later tasks short-circuit once any block has already failed.
    const block_errors = try allocator.alloc(?anyerror, tasks.len);
    defer allocator.free(block_errors);
    for (block_errors) |*e| e.* = null;
    ...
    const Runner = struct {
        fn runScope(
            scope: anytype,
            alloc: Allocator,
            tasks_in: []const VerifyMultiBlockTask,
            cache: ?*xmss.PublicKeyCache,
            out: []?anyerror,
            failed: *std.atomic.Value(bool),
        ) Allocator.Error!void {
            for (tasks_in, 0..) |*task, i| {
                try scope.spawn(runOne, .{ alloc, task, cache, &out[i], failed });
            }
        }

        fn runOne(
            alloc: Allocator,
            task: *const VerifyMultiBlockTask,
            cache: ?*xmss.PublicKeyCache,
            out: *?anyerror,
            failed: *std.atomic.Value(bool),
        ) void {
            if (failed.load(.acquire)) return; // a sibling block already failed
            verifySignatures(alloc, task.state, task.signed_block, cache, task.precomputed_block_root) catch |err| {
                out.* = err;
    ...
    // `scope` blocks until every spawned verifySignatures() completes (or
    // early-skips after a failure), then propagates any spawn-time OOM.
    try thread_pool.scope(Runner.runScope, .{ allocator, tasks, pubkey_cache, block_errors, &any_failed });

    // First failing block fails the whole batch (all-or-nothing contract).
    for (block_errors) |maybe_err| {
