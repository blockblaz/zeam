...
test "API endpoints - health and justified checkpoint" {
    ...
    // Wait longer for blocks to be processed and chain to be fully initialized
    std.debug.print("Waiting for blocks to be processed and chain initialization...\n", .{});
    ...
    std.debug.print("Justified endpoint response:\n{s}\n", .{justified_response});

    // Check if we got 503 (chain not initialized) or 200 OK
    if (std.mem.indexOf(u8, justified_response, "503 Service Unavailable") != null) {
        std.debug.print("⚠ Chain not initialized yet, retrying after additional wait...\n", .{});
        zeam_request.freeResponse(justified_response);

        // Wait more and retry
        std.time.sleep(10 * std.time.ns_per_s);
        const retry_response = try zeam_request.getJustified();
        defer zeam_request.freeResponse(retry_response);

        std.debug.print("Retry response:\n{s}\n", .{retry_response});
        try testing.expect(std.mem.indexOf(u8, retry_response, "200 OK") != null);

        // Continue with the retry response for validation
        try testing.expect(std.mem.indexOf(u8, retry_response, "application/json") != null);
        try testing.expect(std.mem.indexOf(u8, retry_response, "\"root\"") != null);
        try testing.expect(std.mem.indexOf(u8, retry_response, "\"slot\"") != null);

        const body_start = std.mem.indexOf(u8, retry_response, "\r\n\r\n");
        if (body_start) |start| {
            const json_body = retry_response[start + 4 ..];
            std.debug.print("Justified checkpoint response: {s}\n", .{json_body});

            try testing.expect(json_body.len > 0);
            try testing.expect(json_body[0] == '{');

            const parsed = try std.json.parseFromSlice(std.json.Value, testing.allocator, json_body, .{});
            defer parsed.deinit();
            ...
            std.debug.print("✓ Justified checkpoint: slot={d}, root={s}\n", .{ slot.integer, obj.get("root").?.string });
        }

        std.debug.print("SUCCESS: API endpoints test completed (after retry)\n", .{});
        return;
    }
