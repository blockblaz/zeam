...
pub const NodeCommand = struct {
    ...
    /// `1` keeps devnet5 / leanSpec liveness by allowing singleton aggregates
    /// when block production needs them. Higher values are an explicit zeam
    /// performance optimization that trades slot latency for fewer sub-threshold
    /// aggregates on chatty subnets. See issue #907 finding 4.
    ...
    pub const __messages__ = .{
        ...
        .@"min-aggregation-inputs" = "Minimum (children + gossip-sig) inputs required before the aggregator invokes the recursive STARK prover for an AttestationData. Default 1 keeps the devnet5/leanSpec liveness path spec-aligned by allowing singleton aggregates when block production needs them. Higher values are an explicit zeam performance optimization that skips sub-threshold aggregates on chatty subnets.",
