...
async fn start_network(&mut self,self_port: i32, connect_port: i32) -> (libp2p::swarm::Swarm<Behaviour>, mpsc::UnboundedReceiver<SwarmCommand>) {
...
async fn run_eventloop(&mut self, mut swarm: libp2p::swarm::Swarm<Behaviour>, mut command_rx: mpsc::UnboundedReceiver<SwarmCommand>) {
    loop {
        tokio::select! {
